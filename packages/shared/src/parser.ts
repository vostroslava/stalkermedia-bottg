...
export function calculateResult(answers: UserAnswer[], matrix: TestMatrix): CalculatedResult {
    ...
    // Если есть несколько победителей (смешанный типаж), объединяем их
    // Сортируем для стабильного вывода
    winners.sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));

    // Ограничиваем только двумя типажами для лаконичности, если их больше 2
    const topWinners = winners.slice(0, 2);

    const combinedTypes = topWinners.map(([t]) => t).join('/');

    // Собираем название и описание из всех победителей
    const resultDefs = topWinners.map(([t]) => matrix.results.find((r) => r.type === t)).filter(Boolean) as ResultDefinition[];

    const combinedTitle = resultDefs.map(r => r.title).join('/');

    // Специальная обработка для Хомяка и Лисы по ТЗ (ХомоЛис)
    let finalTitle = combinedTitle;
    if ((combinedTypes === 'hamster/fox' || combinedTypes === 'fox/hamster')) {
        finalTitle = 'ХомоЛис';
    }

    const combinedDescription = `Гибридный профиль.\n\n` +
        resultDefs.map(r => `**${r.title}**: ${r.description}`).join('\n\n');

    return {
        result_type: combinedTypes,
        result_title: finalTitle,
        result_description: combinedDescription,
        scores: scoresObject,
    };
}
